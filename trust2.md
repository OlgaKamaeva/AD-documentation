...
В следующей таблице перечислены наиболее важные бэкенды idmap со ссылками на их документацию. Нажмите на соответствующую ссылку, чтобы узнать, как настроить каждый бэкенд idmap:
...
+ SSSD требует ручного указания настроек для каждого трастового домена в клиентской конфигурации. Это необходимо, так как на текущий момент интерфейс (UI) для настройки трастов не поддерживает такие настройки. Таким образом, все конфигурации должны быть выполнены вручную в файле конфигурации SSSD (sssd.conf).
