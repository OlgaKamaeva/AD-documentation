...
### Winbind
Winbind поддерживает несколько механизмов аутентификации, включая:

+ Kerberos — используется для безопасной аутентификации через билеты Kerberos. Это основной механизм при работе с доменами Active Directory. Winbind автоматически получает и управляет Kerberos-билетами для пользователей.
...
### SSSD
SSSD поддерживает кеширование учетных данных и аутентификационных токенов для работы в офлайн-режиме.

Параметр для кеширования в SSSD:

+ cache_credentials = True
Определяет, должны ли учетные данные пользователя также сохраняться в локальном кэше LDB. Кэшированные учетные данные относятся к паролям, включая первый (долгосрочный) фактор двухфакторной аутентификации, но не другие механизмы аутентификации.  По умолчанию: FALSE

### Winbind

В Winbind используется внутренний кеш, который хранит информацию о пользователях и группах, а также результатах аутентификации. Эта информация сохраняется в файле кеша winbindd_cache.tdb.

Параметры конфигурации для кеширования в Winbind:
...
Для корректной работы клиентской машины в домене важно не только настроить аутентификацию и авторизацию, но и обеспечить правильное обновление записей в DNS и настройку доверительных отношений, если требуется взаимодействие с другими доменами.
## Обновление DNS
...
| **Кэширование учетных данных**    |В Winbind используется внутренний кеш, который хранит информацию о пользователях и группах, а также результатах аутентификации. Эта информация сохраняется в файле кеша winbindd_cache.tdb. <br>**Параметры конфигурации для кеширования в Winbind**: winbind offline logon = yes | SSSD поддерживает кеширование учетных данных и аутентификационных токенов для работы в офлайн-режиме. <br> **Параметр для кеширования в SSSD**: cache_credentials = True|
