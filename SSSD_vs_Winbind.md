...
| **Поддержка доверительных отношений типа External** |  Поддерживает. По умолчанию не обнаруживает доверенные домены автоматически. Необходим параметр  winbind scan trusted domains = yes.| Поддерживает. Требует явного указания всех доменов в sssd.conf, даже если доверие уже существует.|
| **Поддержка кросс-лесных доверительных отношений** | Поддерживает. Автоматически видит все домены другого леса при наличии доверия леса.|Не поддерживает. |
...
### SSSD
**Работа с несколькими доменами**: Если один домен доверяет другому, SSSD может быть настроен для работы с несколькими доменами и их пользователями, обеспечивая доступ через одну машину. Для этого требуется явное указание всех доменов в sssd.conf. Подробнее https://www.altlinux.org/ActiveDirectory/Trusts#SSSD.
